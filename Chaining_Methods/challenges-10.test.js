...
const createServer = () => {
    ...
    app.get('/aboutme', myBio);
    app.get('/favoritefoods', foodWhichIlove);
...
function sayHello(eq,res){
    res.status(200).send('Hello from Kate');
}

function myBio(req,res){
    res.status(200).send('My name is Kate, I am full stack software developer. I am prepering for my 401 JS claas and can\'t wait to start');
}

function foodWhichIlove(req,res){
    res.status(200).send('I love sushi, mexican and Indian food ;)')
}
/* ------------------------------------------------------------------------------------------------
CHALLENGE 2
...
const count = (target, input) => {
    let newAr = input.flat();
    let out = newAr.filter(num => num === target);
    // console.log(out, newAr)
    return out.length;
  };
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const divisibleByFiveTwoToThePower = (input) => {
    ...
    for(let i =0; i< input.length; i++){
      if(input[i].length === 0){
        ...
        out.push(input[i].filter(num => num % 5 === 0).filter(a => typeof(a) !== 'string').map(num=> Math.pow(2,num)))
        }
      }
    return out;
  };
...
let findMaleAndFemale = (data) => {
    return data.filter(obj => obj.gender.includes("male")).map(char => char.name).join(" and ")
};
...
let findShortest = (data) => {
    return data.map(char => char.name).reverse().sort((a, b) => a.length - b.length)[0]
};
